// ============================================================
  ...
  const state = {
    puck: { x: 0.5, y: 0.5, vx: 0.00022, vy: 0.00042 },
    mallets: [{ x: 0.5, y: 0.80 }, { x: 0.5, y: 0.20 }]
  };
  ...
  function resetPuck(direction = Math.random() > 0.5 ? 1 : -1) {
    ...
    state.puck.vx = Math.sin(angle) * 0.00028;
    state.puck.vy = direction * Math.cos(angle) * 0.00038;
  }
  ...
  function moveAI() {
    ...
    ai.x += (target.x - ai.x) * 0.045;
    ai.y += (Math.min(0.45, Math.max(0.08, target.y)) - ai.y) * 0.055;
    ai.x = Math.max(0.10, Math.min(0.90, ai.x));
    ai.y = Math.max(0.08, Math.min(0.45, ai.y));
  }
  ...
  function moveMalletsByKeyboard(dt) {
    ...
    state.mallets[0].x = Math.max(0.10, Math.min(0.90, state.mallets[0].x + dx1));
    state.mallets[0].y = Math.max(0.54, Math.min(0.90, state.mallets[0].y + dy1));

    if (!vsAI) {
      state.mallets[1].x = Math.max(0.10, Math.min(0.90, state.mallets[1].x + dx2));
      state.mallets[1].y = Math.max(0.08, Math.min(0.45, state.mallets[1].y + dy2));
    }
  }
  ...
  // ============================================================
  function bindTouch(el, index, minY, maxY, condition) {
    let active = false;
    ...
    el.addEventListener('touchmove', (e) => {
      ...
      const t = e.touches[0];
      const pos = getPos(t.clientX, t.clientY);
      let x = Math.max(0.10, Math.min(0.90, pos.x));
  ...
  // Jogador 1 (mallet rosa) – toque apenas no próprio mallet
  bindTouch(malletEls[0], 0, 0.54, 0.90, null);
  // Jogador 2 (mallet roxo) – toque na mesa, mas só ativa se estiver na faixa superior (8%~45%)
  bindTouch(table, 1, 0.08, 0.45, (x, y) => y >= 0.08 && y <= 0.45);
  ...
  function loop(now) {
    ...
    if (!paused) {
      ...
      if (state.puck.x < 0.035) { state.puck.x = 0.035; state.puck.vx *= -1; }
      if (state.puck.x > 0.965) { state.puck.x = 0.965; state.puck.vx *= -1; }

      collideMallet(0);
      collideMallet(1);

      if (state.puck.y < -0.01) {
        scorePoint(0);
      } else if (state.puck.y > 1.01) {
        scorePoint(1);
      } else {
        if (state.puck.y < 0.035 && Math.abs(state.puck.x - 0.5) > 0.22) {
          state.puck.y = 0.035; state.puck.vy = Math.abs(state.puck.vy);
        }
        if (state.puck.y > 0.965 && Math.abs(state.puck.x - 0.5) > 0.22) {
          state.puck.y = 0.965; state.puck.vy = -Math.abs(state.puck.vy);
        }
      }
  ...
  document.getElementById('newGame').addEventListener('click', () => {
    ...
    state.mallets[0] = { x: 0.5, y: 0.80 };
    state.mallets[1] = { x: 0.5, y: 0.20 };
    setActive(0);
    resetPuck();
